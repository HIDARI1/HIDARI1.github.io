function blackhole(element) {
    ...
    class Star {
        constructor() {
            ...
            // Convert HSL to RGB for the violet color (hsl(240, 69%, 61%))
            const opacity = 1 - ((this.orbital) / 255);
            // hsl(240, 69%, 61%) = rgb(48, 63, 159) approximately
            // Using a brighter violet for better visibility
            const h = 240;
            const s = 69;
            const l = 61 + (opacity * 20); // Slightly brighter for outer stars
            this.color = `hsla(${h}, ${s}%, ${Math.min(l, 85)}%, ${opacity})`; // Color the star violet, but make it more transparent the further out it is generated
    ...
    centerHover.addEventListener('click', function() {
        ...
        // Rediriger vers index.html après l'animation d'expansion
        setTimeout(() => {
            window.location.href = 'index.html';
        }, 2000); // 2 secondes d'animation avant la redirection
    });
    ...
    function loop() {
        ...
        context.fillStyle = 'rgba(18, 18, 30, 0.2)'; // somewhat clear the context with violet tint, this way there will be trails behind the stars
    ...
    function init() {
        context.fillStyle = 'hsl(240, 28%, 12%)'; // Initial clear of the canvas with violet background
        context.fillRect(0, 0, cw, ch);
